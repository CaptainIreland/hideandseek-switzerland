...
const ASSUMPTIONS = [
  ...
  {id:"core-uk-english", section:"core", title:"UK English, no em dashes",
   body:"Applies everywhere in this project, including code comments and interface text (this page follows the same rule).",
   tags:[]},
  ...
  {id:"q-thermo", section:"questions", title:"Thermometer is a half-plane, warned but never blocked",
   body:"A perpendicular bisector splits the map; hotter keeps the half toward the end point. The card played (the minimum distance the asker must have travelled for the question to be valid) is compared live against the actual straight-line distance between the two points. Falling short only warns, since the geometry is valid either way.",
   tags:["rulebook"]},
  ...
  {id:"q-units", section:"questions", title:"Imperial and metric are parallel rule sets, not conversions",
   body:"The Large-game question pad has two official value sets (10 mi radar and 15 km radar are different cards, not the same distance in two units). Toggling units never touches an existing clue: each clue's label is baked in at add time from whatever unit was active then, and stays that way regardless of later toggling.",
   tags:["units","clues"]},

  // --- Data thresholds & filtering ---
  {id:"t-min-reviews", section:"thresholds", title:"Minimum Google review counts, per category",
   body:"Hospital 50, airport 100, museum 10, park and amusement park 20, library, cinema and golf 5, zoo 10, aquarium 0, consulate 5. Review count turned out to be the only strong signal for whether a place is the real thing, since type filtering alone barely helps (Google's primary type for a dental surgery is genuinely \"hospital\").",
  ...
  {id:"s-version-bump", section:"sharing", title:"SHARE_VERSION bumps whenever the wire format changes",
   body:"Currently 3, for the thermometer card fields and the station clue type. Old links just show a graceful \"this build does not understand that format\" banner rather than misparsing.",
   tags:["sharing"]},
