...
BBOX = (5.9, 45.8, 10.6, 47.85)  # lng_min, lat_min, lng_max, lat_max

# Swiss outline (about 100 m tolerance), [lng, lat] ring.
...
def ring_dist_km(lat, lng):
    best = float("inf")
    px = lng * math.cos(math.radians(lat))
    prev = CH_RING[-1]
    for cur in CH_RING:
        x1, y1 = prev[0] * math.cos(math.radians(lat)), prev[1]
        x2, y2 = cur[0] * math.cos(math.radians(lat)), cur[1]
        dx, dy = x2 - x1, y2 - y1
        if dx == 0 and dy == 0:
            t = 0.0
        else:
            t = max(0.0, min(1.0, ((px - x1) * dx + (lat - y1) * dy) / (dx * dx + dy * dy)))
        qx, qy = x1 + t * dx, y1 + t * dy
        d = math.hypot(px - qx, lat - qy) * 111.32
        if d < best:
            best = d
        prev = cur
    return best
...
def cell_touches_ch(clat, clng, dlat, dlng, samples):
    for i in range(samples):
        for j in range(samples):
            lat = clat - dlat / 2 + dlat * i / (samples - 1)
            lng = clng - dlng / 2 + dlng * j / (samples - 1)
            if point_in_ch(lng, lat):
                return True
    return False


def cell_radius_m(clat, dlat, dlng):
    height = dlat * 111320.0
    width = dlng * 111320.0 * max(0.2, math.cos(math.radians(clat)))
    return 0.5 * math.hypot(height, width) * 1.02


def initial_cells():
    dlat, dlng = metres_to_deg(START_SIDE_M, 46.8)
    cells = []
    lat = BBOX[1] + dlat / 2
    while lat < BBOX[3] + dlat / 2:
        lng = BBOX[0] + dlng / 2
        while lng < BBOX[2] + dlng / 2:
            if cell_touches_ch(lat, lng, dlat, dlng, 9):
                cells.append((lat, lng, dlat, dlng))
            lng += dlng
        lat += dlat
    return cells
...
def search(key, lat, lng, radius_m):
    headers = {
    ...
    body = {
        ("includedPrimaryTypes" if globals().get("USE_PRIMARY_TYPES") else "includedTypes"): SEARCH_TYPES,
        "maxResultCount": 20,
        "locationRestriction": {"circle": {
            "center": {"latitude": round(lat, 6), "longitude": round(lng, 6)},
            "radius": min(float(radius_m), 50000.0),
        }},
    }
    for attempt in (1, 2):
        try:
            return _post(API, body, headers)
        except urllib.error.HTTPError as error:
            detail = error.read().decode(errors="replace")[:300]
            if error.code in (400, 401, 403):
                sys.exit(
                    f"Google refused the request (HTTP {error.code}).\n"
                    "Checks: Places API (New) enabled on the project, billing enabled, and the key's\n"
                    "application restriction set to None (website-restricted keys reject script calls).\n"
                    f"Detail: {detail}")
            if attempt == 2:
                raise
            time.sleep(2)
...
def sweep(key):
    places = {}
    queue = list(initial_cells())
    top_level = len(queue)
    requests_made = 0
    warnings = []
    while queue:
        clat, clng, dlat, dlng = queue.pop()
        radius = cell_radius_m(clat, dlat, dlng)
        data = search(key, clat, clng, max(radius, 50.0))
        requests_made += 1
        time.sleep(0.12)
        ...
        if requests_made % 25 == 0:
            print(f"  {requests_made} requests, {len(places)} unique places so far", flush=True)
        if len(results) >= 20:
            side_m = dlat * 111320.0
            if side_m / 2 < MIN_SIDE_M:
                warnings.append(f"Dense cell still capped at {int(side_m)} m near {clat:.4f}, {clng:.4f}")
                continue
            for qa in (-0.25, 0.25):
                for qb in (-0.25, 0.25):
                    child = (clat + dlat * qa, clng + dlng * qb, dlat / 2, dlng / 2)
                    if cell_touches_ch(child[0], child[1], child[2], child[3], 5):
                        queue.append(child)
    return places, requests_made, top_level, warnings
...
def partition(places):
    ...
    for place in places.values():
        ...
        outline_says = point_in_ch(lng, lat)
        ...
        if not swiss:
            record["borderKm"] = round(ring_dist_km(lat, lng), 2)
...
def audit_core(kept):
    ...
    for name, lat, lng in core:
        if name.strip().lower() in names:
            matched += 1
        elif any(dist_km((lat, lng), p) < 0.3 for p in points):
            matched += 1
        else:
            missing.append(name)
...
def main():
    ...
    places, requests_made, top_level, warnings = sweep(key)
