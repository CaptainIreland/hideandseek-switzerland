"""Shared adaptive-cell sweep against the Google Places API (New) searchNearby
endpoint, plus the Swiss outline check the sweep clips against.

Used by fetch_stations_google.py (train and light-rail stations) and
fetch_places_google.py (every other place category), so both get identical
Swiss-outline clipping, 20-result-cap subdivision, and retry behaviour instead
of one reaching into the other's module globals.
"""
import json
import math
import time
...
# Same sampling density is used to test whether a cell touches Switzerland at
# both the top level and every subdivision, so a thin border slice is equally
# unlikely to be missed regardless of how deep the subdivision has gone.
CELL_TOUCH_SAMPLES = 9
...
def cell_touches_ch(clat, clng, dlat, dlng, samples=CELL_TOUCH_SAMPLES):
...
def initial_cells(bbox=BBOX, start_side_m=START_SIDE_M):
    ...
    while lat < bbox[3] + dlat / 2:
        ...
        while lng < bbox[2] + dlng / 2:
            if cell_touches_ch(lat, lng, dlat, dlng):
...
def search(key, lat, lng, radius_m, field_mask, search_types, use_primary_types=False, api_url=API):
    headers = {
    ...
    body = {
        ("includedPrimaryTypes" if use_primary_types else "includedTypes"): search_types,
        "maxResultCount": 20,
    ...
    for attempt in (1, 2):
        try:
            return _post(api_url, body, headers)
            ...
            if error.code in (400, 401, 403):
                sys_exit_message = (
                    ...
                    f"Detail: {detail}")
                raise SystemExit(sys_exit_message)
            if attempt == 2:
                raise
            time.sleep(2)
        except Exception:
...
def sweep(key, search_types, field_mask, use_primary_types=False, *,
          bbox=BBOX, start_side_m=START_SIDE_M, min_side_m=MIN_SIDE_M, api_url=API,
          on_progress=None, checkpoint_every=25):
    """Adaptive-cell sweep. Returns (places, requests_made, top_level, warnings).

    `places` is a dict keyed by Google place id, accumulated as the sweep runs.
    If given, on_progress(places, requests_made) is called every
    checkpoint_every requests, so a caller can persist a recovery snapshot of
    the (already paid-for) raw results without waiting for the whole sweep -
    normally several thousand requests - to finish.
    """
    places = {}
    queue = list(initial_cells(bbox, start_side_m))
    top_level = len(queue)
    requests_made = 0
    warnings = []
    while queue:
        ...
        data = search(key, clat, clng, max(radius, 50.0), field_mask, search_types, use_primary_types, api_url)
        requests_made += 1
        time.sleep(0.12)
        ...
        if requests_made % checkpoint_every == 0:
            ...
            if on_progress:
                on_progress(places, requests_made)
        if len(results) >= 20:
            ...
            for qa in (-0.25, 0.25):
                for qb in (-0.25, 0.25):
                    ...
                    if cell_touches_ch(child[0], child[1], child[2], child[3]):
                        queue.append(child)
