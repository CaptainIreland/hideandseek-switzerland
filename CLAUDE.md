...
- **Thermometer**: half-plane on the perpendicular bisector, toward the end point when hotter. The card played (the minimum distance the asker must have travelled for the question to be valid, ½/3/10/50 mi or 1/5/15/75 km) is tracked as `THERMO_CARD_KM` and compared live against the actual straight-line `turf.distance()` between the two points. Falling short only warns, never blocks, since the geometry is valid either way; the measured distance and any shortfall are baked into the clue's label so a mixed-compliance history stays legible.
